...
// BUG FIX #4: Call Fish Audio API directly from frontend to avoid 60s timeout
async function speakWithFishAudio(text, ttsConfig, onEnd) {
  try {
    const fishKey = ttsConfig.api_key;
    const referenceId = ttsConfig.reference_id;
    const model = ttsConfig.model || 's2-pro';
    
    if (!fishKey || !referenceId) {
      console.warn('[Airis TTS] Fish Audio config incomplete, falling back to browser');
      speakBrowser(text, onEnd);
      return;
    }
    
    // Call Fish Audio API directly from frontend
...
// BUG FIX #5: Read voice settings from localStorage
function speakBrowser(text, onEnd) {
  ...
  try {
    ...
    const voiceRate = parseFloat(settings.voice_rate || 1.05);
    const voicePitch = parseFloat(settings.voice_pitch || 0.95);
    const voiceVolume = parseFloat(settings.voice_volume || 1);
    const voiceLanguage = settings.voice_language || 'en-US';
    
    utt.rate = voiceRate;
    utt.pitch = voicePitch;
    utt.volume = voiceVolume;
    utt.lang = voiceLanguage;
  } catch (e) {
    utt.rate = 1.05;
    utt.pitch = 0.95;
    utt.volume = 1;
