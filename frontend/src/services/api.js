...
const tauriShell = {
  openApp: async (appName) => {
    // BUG FIX #7: Platform detection - web doesn't support app launching
    if (!isTauri()) {
      return {
        success: false,
        error: 'App launching is only available in the Airis desktop app. Download from airis-9ox.pages.dev/download'
      };
    }
    try {
      ...
      if (!entry) return { success: false, error: `Unknown app: ${appName}. Available: Chrome, VS Code, Notepad, Explorer, Terminal, PowerShell, Calculator, Settings, Edge, Firefox` };

      const [program, args] = entry;
      await new Command(program, args).execute();
      return { success: true, message: `Opened ${appName}` };
    } catch (e) {
      return { success: false, error: `Failed to open ${appName}: ${e.message}` };
    }
  },
};
  ...
  chat: async (text, appState = null) => {
    ...
    if (groqKey) {
      try {
        ...
        // BUG FIX #3: Include last 10 messages as context for conversation continuity
...
export const api = {
  ...
  // BUG FIX #3: History (persistent localStorage — never lost on refresh)
  saveHistory: (messages) => {
    try {
      const trimmed = messages.slice(-100);
      localStorage.setItem(HISTORY_KEY, JSON.stringify(trimmed));
      localSettings.setHistory(trimmed);
    } catch {}
  },
  loadHistory: () => {
    try {
  ...
  // BUG FIX #3: Restore history on app init
  restoreHistory: () => {
    try {
      const h = localStorage.getItem(HISTORY_KEY);
      return h ? JSON.parse(h) : [];
